#!/usr/bin/env python3
"""Ingest MX Family Datasheet into the knowledge base.

Usage:
    python scripts/ingest_mx_datasheet.py
"""
...
DATASHEET_PATH = "datasheet/MX-family-datasheet-20251016-english.pdf"
DATASHEET_FILENAME = "MX-family-datasheet-20251016-english.pdf"
...
async def delete_existing_datasheet(session):
    """Delete any existing MX Family datasheet document and its chunks."""
    from sqlalchemy import text

    # Find the document
    result = await session.execute(
        text("SELECT id, filename, total_chunks FROM knowledge_documents WHERE filename LIKE '%MX-family-datasheet%' OR filename LIKE '%mx-family-datasheet%'")
    )
    rows = result.fetchall()

    if not rows:
        logger.info("No existing MX Family datasheet found in knowledge base")
        return 0
...
async def ingest_datasheet():
    """Ingest the MX Family datasheet."""
    from src.config.database import get_db
    ...
    async with db.session() as session:
        ...
        # Ingest
        logger.info("\n" + "=" * 60)
        logger.info("STEP 2: Ingesting datasheet")
        logger.info("=" * 60)
        ...
        try:
            document = await ingestion_service.ingest_pdf_document(
                ...
                doc_type="datasheet",
                product="meraki",
                title="Cisco Meraki MX Family Datasheet",
                description="Official Cisco datasheet for Meraki MX security and SD-WAN appliances"
            )
...
async def main():
    logger.info("=" * 60)
    logger.info("MX FAMILY DATASHEET INGESTION")
    logger.info("=" * 60)

    success = await ingest_datasheet()
