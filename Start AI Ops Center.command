...
# Kill any existing AI Ops Center processes and free required ports
cleanup_existing_processes() {
    ...
    if pgrep -f "uvicorn src.api.web_api" > /dev/null 2>&1; then
        print_warning "Found existing backend processes — killing them"
        pkill -9 -f "uvicorn src.api.web_api" 2>/dev/null || true
        cleaned=1
    fi

    # Kill any existing Next.js frontend processes
    if pgrep -f "next-server" > /dev/null 2>&1; then
        print_warning "Found existing frontend processes — killing them"
    ...
    # If ports are still occupied after killing known processes, force-free them
    if check_port 8002; then
        print_warning "Port 8002 still in use — killing remaining processes"
        kill -9 $(lsof -t -i :8002) 2>/dev/null || true
        cleaned=1
    fi

    if check_port 3000; then
        print_warning "Port 3000 still in use — killing remaining processes"
    ...
    if [ $cleaned -eq 1 ]; then
        ...
        print_success "No existing processes found — ports are available"
    fi
}
...
install_mcp_remote() {
    print_step "Checking mcp-remote..."

    if npx mcp-remote --help &> /dev/null 2>&1; then
        print_success "mcp-remote already available"
        return 0
    fi

    print_step "Installing mcp-remote globally..."
    npm install -g mcp-remote >> "$LOG_FILE" 2>&1 || true
    print_success "mcp-remote installed"
}
...
# Build frontend for production (pre-compiles all pages)
build_frontend() {
    print_step "Checking frontend build..."

    cd "$SCRIPT_DIR/web-ui"

    # Skip build if already built and no source changes
    if [ -d ".next" ] && [ -f ".next/BUILD_ID" ] && [ "${CLEAN_BUILD:-}" != "1" ]; then
        CHANGED=""
        # Check if any source files are newer than the build
        CHANGED=$(find src/ public/ -newer .next/BUILD_ID \( -name '*.ts' -o -name '*.tsx' -o -name '*.css' -o -name '*.json' \) 2>/dev/null | head -1)
        # Also check config files
        if [ -z "$CHANGED" ]; then
            for cfg in next.config.ts tailwind.config.ts package.json tsconfig.json; do
                if [ -f "$cfg" ] && [ "$cfg" -nt ".next/BUILD_ID" ]; then
                    CHANGED="$cfg"
                    break
                fi
            done
        ...
        if [ -z "$CHANGED" ]; then
            print_success "Frontend already built (use CLEAN_BUILD=1 to rebuild)"
            cd "$SCRIPT_DIR"
            return
        fi
    ...
    if [ "${CLEAN_BUILD:-}" = "1" ]; then
        print_info "Clean build requested, removing old build..."
...
# Health check for backend - uses public /api/readiness endpoint
check_backend_health() {
    local max_attempts=90  # 90 seconds for embedded postgres startup
    local attempt=1

    while [ $attempt -le $max_attempts ]; do
        # Check the public readiness endpoint - returns 200 only when fully ready
        # Returns 503 while starting up, so we just need to check for HTTP 200
        local http_code
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" --connect-timeout 2 "https://localhost:8002/api/readiness" 2>/dev/null)

        if [ "$http_code" = "200" ]; then
            return 0
        fi

        # Also try HTTP in case SSL isn't configured
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" --connect-timeout 2 "http://localhost:8002/api/readiness" 2>/dev/null)
        ...
        if [ $((attempt % 10)) -eq 0 ]; then
            print_info "Still waiting for backend... ($attempt/$max_attempts)"
        fi
    done
...
# Health check for frontend - verifies actual page load, not just server response
check_frontend_health() {
    local max_attempts=90  # Increased to 90 seconds for cold Next.js compilation
    local attempt=1

    while [ $attempt -le $max_attempts ]; do
        # Check for HTTP 200 status code, not just any response
        local http_code
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" "https://localhost:3000" 2>/dev/null)
        ...
        # Also try HTTP
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" "http://localhost:3000" 2>/dev/null)
        ...
        if [ $((attempt % 15)) -eq 0 ]; then
            print_info "Still waiting for frontend to compile... ($attempt/$max_attempts)"
        fi
    done
...
# Start frontend server (production mode — all pages pre-compiled)
start_frontend() {
    ...
    if [ -f "$MKCERT_CA" ]; then
        NODE_EXTRA_CA_CERTS="$MKCERT_CA" npm start >> "$LOG_FILE" 2>&1 &
    else
        # Fallback: disable TLS verification for localhost backend
        NODE_TLS_REJECT_UNAUTHORIZED=0 npm start >> "$LOG_FILE" 2>&1 &
    fi
    ...
    if check_frontend_health; then
        ...
        print_info ""
        print_info "Tip: Try running with CLEAN_BUILD=1 to force rebuild:"
        print_info "  CLEAN_BUILD=1 ./\"Start AI Ops Center.command\""
        exit 1
    fi
...
open_browser() {
    ...
    # Wait a few seconds for the frontend to fully initialize
    # This prevents showing a loading or 404 page
    ...
    for i in {1..10}; do
        local http_code
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" "https://localhost:8002/api/setup/status" 2>/dev/null)
        if [ "$http_code" = "200" ]; then
            api_ready=1
            break
        fi
        http_code=$(curl -sk -o /dev/null -w "%{http_code}" "http://localhost:8002/api/setup/status" 2>/dev/null)
        if [ "$http_code" = "200" ]; then
            api_ready=1
            break
        fi
    ...
    if [ $api_ready -eq 0 ]; then
        print_warning "API may still be initializing - browser may show loading state"
    fi
...
cleanup() {
    echo ""
    echo -e "${CYAN}Shutting down AI Ops Center...${NC}"
...
main() {
    ...
    # Initialize log
    echo "=== AI Ops Center Startup - $(date) ===" > "$LOG_FILE"
    log "OS: $OS, Arch: $ARCH"
