...
title Lumen - AI-Powered Network Intelligence Platform
...
:: Initialize log file
echo === Lumen Startup - %date% %time% === > "%LOG_FILE%"
...
if %errorlevel% equ 0 (
    for /f "tokens=1 delims=v" %%V in ('node --version') do (
        for /f "tokens=1 delims=." %%M in ("%%V") do (
            if %%M geq 18 (
                set "NODE_OK=1"
                echo %GREEN%√%NC% Found Node.js v%%V
            )
        )
    )
)
...
for /l %%i in (1,1,30) do (
    ...
    curl -sk "%BACKEND_URL%/api/health" >nul 2>&1
    if !errorlevel! equ 0 (
        set "BACKEND_READY=1"
...
if exist "certificates\localhost.pem" if exist "certificates\localhost-key.pem" (
    :: Set NODE_EXTRA_CA_CERTS if we have mkcert CA (Windows stores in LOCALAPPDATA)
